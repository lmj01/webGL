(function() {
    ...
    window.getModelFileByUrl = function(url, callback) {
        var rawFile = new XMLHttpRequest();
        rawFile.open('GET', url, true);
        rawFile.responseType = 'blob';
        rawFile.onloadend = function(){
            loadModel(rawFile.response, null, callback);
        }
        rawFile.send(null);
    }

    window.loadModel = function(file, matrix, callback) {
    ...
    function adjust(vtx, matrix) { // center the model in front of the camera and set an decent starting scale
